...
from FILE_zhelpers import socket_set_hwm, zpipe

CHUNK_SIZE = 250000
SAVE_PATH='/tmp/FILE_zmp_copy.tgz'
...
def client_thread(ctx, pipe):
    ...
    chunkCount = 0      # Total chunkCount received


    destFile = open(SAVE_PATH,'wb')
    ...
    print ("%i chunkCount received, %i bytes" % (chunkCount, total))
    pipe.send(b"OK")
    destFile.close();
    print "file save to ", SAVE_PATH
